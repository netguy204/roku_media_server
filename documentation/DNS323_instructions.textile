...
- Download the Google Git client here:  http://code.google.com/p/msysgit/downloads/list - the top link worked for me
...
2. Join the Roku Developer community - this is so you can get the code in the documents to unlock your Roku box and put it in Dev mode. 
- Download the SDK 
...
- Once everything is on your NAS you run the app (via telnet) by typing in 'python rss_server.py' from the Server directory you uploaded earlier. There's a way to have this run on startup via FFP on the D-Link but I haven't configured it yet. I'm sure this can be done on the Maxtor as well but I'm not sure how. To have python continue without your telnet window open you can type  'python rss_server.py &' 

4. Troubleshooting

Verify library is working.. you should see songs in here:
http://NAS_IP_Address_Here:8001/feed

Check the Roku debug port to see what's going on while it loads/runs the MyMusic client:
http://ROKU_IP_HERE:8085/
